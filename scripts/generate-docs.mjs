 ...
 *   8. Public Copy (llms files → playground/public/)
 *   9. Component MDX Pages (docs/content/docs/components/*.mdx + meta.json)
 */
...
// ── GENERATOR 9: Component MDX pages (docs/content/docs/components/) ────────

const DOCS_COMPONENTS_DIR = path.resolve(ROOT, 'docs/content/docs/components');
const MDX_MARKER =
  '{/* @generated by scripts/generate-docs.mjs from manifest.ai.json - do not edit by hand */}';
const MDX_CATEGORY_ORDER = ['primitives', 'composites', 'layout', 'patterns', 'context', 'other'];
const MDX_CATEGORY_TITLES = {
  primitives: 'Primitives',
  composites: 'Composites',
  layout: 'Layout',
  patterns: 'Patterns',
...
/** PascalCase → kebab-case, acronym-aware (KPICard → kpi-card) */
...
function mdxTableCell(value) {
  return String(value).replace(/\|/g, '\\|').replace(/\r?\n/g, ' ').trim();
}

function componentMdx(component) {
  const { name, category, import: importLine, props } = component;
  const lines = [
    '---',
    `title: ${name}`,
    `description: API reference for the ${name} component (${category}) in @arcana-ui/core.`,
    '---',
    '',
    MDX_MARKER,
    '',
  ...
  if (propEntries.length > 0) {
    ...
    for (const [propName, def] of propEntries) {
      const type = `\`${mdxTableCell(def.type ?? 'unknown')}\``;
      const fallback = def.required === false ? '-' : '(required)';
      const defaultValue =
        def.default === undefined ? fallback : `\`${mdxTableCell(JSON.stringify(def.default))}\``;
      const description = mdxTableCell(def.description ?? '');
      lines.push(`| \`${propName}\` | ${type} | ${defaultValue} | ${description} |`);
    }
    lines.push('');
  } else {
    lines.push(
      '## Props',
      '',
      `No props are documented in \`manifest.ai.json\` for ${name}. The component supports \`className\` passthrough and the standard HTML attributes of its underlying element.`,
      '',
    );
  }
...
function runComponentMdxGenerator(manifest) {
  console.log('  [9/9] Component MDX Pages (docs/content/docs/components/)...');
  ...
  for (const component of components) {
    ...
    const filePath = path.resolve(DOCS_COMPONENTS_DIR, `${slug}.mdx`);
    if (fs.existsSync(filePath)) {
      const existing = fs.readFileSync(filePath, 'utf8');
      if (!existing.includes('@generated by scripts/generate-docs.mjs')) {
        preserved += 1;
        continue;
      }
      const next = componentMdx(component);
      if (existing === next) {
        unchanged += 1;
        continue;
      }
    ...
    writeFile(filePath, componentMdx(component));
    generated += 1;
  }

  // Sidebar: hand-written and generated pages together, grouped by category
  const pages = [];
  for (const category of MDX_CATEGORY_ORDER) {
    const group = components
      .filter((c) => c.category === category)
      .sort((a, b) => a.name.localeCompare(b.name));
    if (group.length === 0) continue;
    pages.push(`---${MDX_CATEGORY_TITLES[category] ?? category}---`);
    pages.push(...group.map((c) => componentSlug(c.name)));
  }
  // Rest operator keeps hand-written pages whose slug has no manifest
  // counterpart (e.g. toast.mdx vs ToastProvider) visible in the sidebar
  pages.push('...');
  writeJSON(path.resolve(DOCS_COMPONENTS_DIR, 'meta.json'), { title: 'Components', pages });

  console.log(
    `         ${generated} generated, ${unchanged} unchanged, ${preserved} hand-written preserved, ${pages.length} sidebar rows`,
  );
}
...
async function main() {
  ...
  runVersionSync();
  runPublicCopy();
  runComponentMdxGenerator(manifest);

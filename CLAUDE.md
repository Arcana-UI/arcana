...
### Active Phase
Tree-shaking fix shipped to `develop` (5.9 closed). v0.1.0 shipped 2026-04-16 (PR #118), v0.1.1 hotfix shipped 2026-04-17 (PRs #120/#121) restoring CSS module class-name maps in the published `@arcana-ui/core` bundle. v0.1.2 staged on `develop` with per-component entry points + per-subpath exports map; awaiting Bear to cut the release. All four packages on npm: tokens, cli, mcp at 0.1.0; core at 0.1.1 published, 0.1.2 staged.

The April 15-17 sprint closed five fronts: KV→Supabase cache swap (#114), three-PR landing polish + per-preset motion personalities (#115, #116, #117), useTheme test repair, and the CSS-module hotfix release. The May 7 session closed the post-pause audit, the cleanup PR (#122), and the tree-shaking fix.

Tree-shaking is no longer a launch blocker. Single Button consumer import drops from 278 kB to 2.2 kB minified (99.2% reduction). Full barrel import drops from 278 kB to 166.9 kB (consumers that don't deep-import still benefit from the new architecture). Verified end-to-end with a fresh `/tmp/treeshake-test/` consumer fixture using a packed `.tgz` and esbuild bundle.
...
### Blockers
**0.1.2 release pending.** Bear needs to: (1) review and merge `perf/5.9-per-component-entry-points` to develop (or whatever lands first), (2) cut a release branch from develop, (3) tag `v0.1.2` on the merge commit, (4) `npm publish` `@arcana-ui/core@0.1.2` from main, (5) sync develop with main. Same flow as 0.1.1.

### What the Next Agent Should Do
1. Read CLAUDE.md, PROGRESS.md, ROADMAP.md, AI_OPS.md.
2. Priority: **5.12 DESIGN.md export.** Validate against `@google/design.md` CLI in CI. Each of the 14 presets gets a DESIGN.md file. PR'd to VoltAgent's `awesome-claude-design` list. Driven by Google Labs spec release 2026-04-21; the standard is fresh and being late on it costs positioning. See `~/Documents/Claude/Projects/Arcana/COMPETITIVE_INTEL_2026-05-07.md` insertion 1.
3. Alternate priorities: P.14 (Claude Design integration pack: `@arcana-ui/claude-design-pack` + landing page at `arcana-ui.com/claude-design`), 5.5 (finish docs site scaffold and wire content from manifest.ai.json).

...
### Active Phase
Pre-launch cleanup followed by tree-shaking fix (5.9). v0.1.0 shipped 2026-04-16 (PR #118), v0.1.1 hotfix shipped 2026-04-17 (PRs #120/#121) restoring CSS module class-name maps in the published `@arcana-ui/core` bundle (P0: every component was rendering unstyled before). All four packages live on npm: tokens, cli, mcp at 0.1.0; core at 0.1.1.

The April 15-17 sprint closed five fronts in three days: KV→Supabase cache swap (#114), three-PR landing polish + per-preset motion personalities (#115, #116, #117), useTheme test repair, and the CSS-module hotfix release. After that, Bear stepped away for ~3 weeks. This session is the post-pause audit + cleanup.

Tree-shaking is the only remaining technical launch blocker. A single Button import currently ships 278 kB; target is <50 kB. Fix is per-component entry points + per-subpath exports map, not flipping the tsup `splitting` flag (`splitting: true` strips the `"use client"` banner Next.js RSC needs).
...
| Phase 5 — AI Integration & Launch | 🔄 Partial | 5.1-5.4, 5.11 done; 5.5 scaffolded; 5.9 active; 5.12 (DESIGN.md export) new |
...
- **5.9 — Tree-shaking fix. Active task. This is what comes after PR 1.**

...
### Active Phase
Resumed 2026-07-02 after a 7-week pause (last merge was PR #128 on 2026-05-11). The May 7-11 sprint closed the tree-shaking fix (5.9, PR #123), the v0.1.2 release (PR #124, `@arcana-ui/core@0.1.2` live on npm), workflow polish (PR #125), DESIGN.md export (5.12, PR #126), the Claude Design pack (P.14, PR #127), and the release.yml publish step for the pack (PR #128).

`develop` is 7 commits ahead of `main` and 0 behind. Everything is merged: no open PRs, and every historical branch was verified merged on 2026-07-02 (the sole exception is one stray commit on the old `claude/audit-css-properties-T4lBt` scratch branch adding 145 lines to ROADMAP.md that was never PR'd; Bear to review or discard).

`@arcana-ui/claude-design-pack@0.1.0` is **live on npm, published 2026-05-11** (manually by Bear, minutes before PR #128 merged; the release.yml publish step from #128 has not been exercised yet since no release has been cut since). The `/claude-design` playground page is live. A 2026-07-02 audit misread the npm check and recorded the pack as unpublished; corrected 2026-07-24.

Tree-shaking numbers, for reference: single Button consumer import 278 kB → 2.2 kB minified (99.2% reduction); full barrel 278 kB → 166.9 kB.
...
| Phase 3 — Expanded Component Library | ✅ Complete | 108 components per manifest.ai.json |
| Phase 4 — Theme Presets & Demo Sites | ✅ Complete | 14 presets, 6 demos deployed |
| Phase P — Playground Product | 🔄 Partial | P.1-P.5, P.5.1, P.9, P.13, P.14 done; P.5.2/.3, P.6-P.12 deferred |
| Phase 5 — AI Integration & Launch | 🔄 Partial | 5.1-5.4, 5.9, 5.11, 5.12 done; 5.5 scaffolded; 5.6, 5.8, 5.10 open |

### Key Milestones Reached
- **v0.1.2 published** on npm. `@arcana-ui/core@0.1.2` ships per-component entry points for tree-shaking; tokens, cli, mcp remain at 0.1.0. First fully-green release.yml run.
- **DESIGN.md export shipped** (5.12, PR #126). `arcana-ui export-design-md <preset|all>` emits Google DESIGN.md spec-compliant files; CI job validates all 14 presets against `@google/design.md lint` on every PR.
- **Claude Design pack built** (P.14, PR #127). `@arcana-ui/claude-design-pack` with 14 pre-generated DESIGN.md files, `manifest.claude-design.json`, drift-guard CI, and the `/claude-design` playground page. Awaiting npm publish.
- **AI theme generation** live: "Describe your brand. Get a design system." via Anthropic API, BYOK, Supabase semantic cache, error forwarding, IP rate limiting.
- **14 theme presets** with **per-preset motion personalities** (P.13 closed via #117): light, dark, terminal, retro98, glass, brutalist, corporate, startup, editorial, commerce, midnight, nature, neon, mono. Manifest captures motion personality so AI agents can query "which presets have calm motion".
...
- **Landing page** at `arcana-ui.com` with token-driven motion primitives (FadeIn, Stagger, CountUp, GradientBorder) and enterprise polish (PR #116).
- **6 demo sites** deployed: Forma, Wavefront, Mosaic, Atelier, Control, Dashboard.
- **Documentation site** scaffolded under `docs/` (fumadocs-mdx + Next.js 15 + React 19); content generation from manifest.ai.json not yet wired.
- **CLI** (`@arcana-ui/cli@0.1.0`): `init` (5 starter layouts × Vite/Next), `validate` (WCAG checks), `add-theme` (14 presets).
- **MCP server** (`@arcana-ui/mcp@0.1.0`): 7 tools for AI agents.
...
- P.6-P.12 — Theme gallery, OAuth, monetization, accessibility panel. **All deferred.**
- P.14 follow-through - PR the pack to VoltAgent's `awesome-claude-design` list (not yet done as of 2026-07-24; the pack itself published to npm 2026-05-11).

**Phase 5:**
- 5.5 — Documentation site (scaffold deployed, content TBD, auto-generate from manifest.ai.json)
- 5.6 — SEO & discoverability
- 5.8 — Figma Code Connect (light pass scoped post-Mike-Krieger Figma board exit)
- 5.10 — Launch checklist

**Phase 7:** External starter repos and website clone rebuilds. Post-launch.

**Phase 8:** GTM, README overhaul, Show HN.

### Blockers
No hard blockers. `@arcana-ui/claude-design-pack@0.1.0` published 2026-05-11, so nothing is pending npm. Housekeeping: `develop` is 7 commits ahead of `main` (#125-#128 plus the back-merge); the next release cut syncs them and exercises the new release.yml pack-publish step for the first time. The VoltAgent `awesome-claude-design` list PR remains unopened.

### What the Next Agent Should Do
1. Read CLAUDE.md, PROGRESS.md, ROADMAP.md, AI_OPS.md.
2. Priority: **5.5 documentation site content.** The scaffold (fumadocs-mdx + Next.js 15) is deployed at docs.arcana-ui.com; wire content generation from manifest.ai.json.
3. Alternate priorities: 8.1 README overhaul (increasingly conspicuous post-Claude-Design), 5.6 SEO & discoverability, and the P.14 follow-through items above (release is Bear's, the VoltAgent list PR can be drafted for him).

### Session History

| Date | Agent | Tasks Completed | Notes |
|------|-------|-----------------|-------|
| 2026-07-02 | Claude (Claude Code) | Post-pause status audit + tracker refresh | Bear returned after a 7-week pause (last merge PR #128, 2026-05-11). Confirmed all work merged: no open PRs, `develop` 7 ahead / 0 behind `main`, every historical branch content-verified against develop (squash merges make commit-ancestry checks useless, so branches were checked by PR state and content diff). Sole stray: `claude/audit-css-properties-T4lBt` carries one un-PR'd commit adding 145 lines to ROADMAP.md; flagged for Bear to review or discard. npm check: core 0.1.2, tokens/cli/mcp 0.1.0. (This session also recorded `@arcana-ui/claude-design-pack` as NOT published - that was a misread of the npm output; the pack has been live since 2026-05-11. Corrected 2026-07-24.) Also found: no VoltAgent `awesome-claude-design` list PR was ever opened (P.14 follow-through), and `.github/pull_request_template.md` referenced by this file does not exist in the repo (only `workflows/` is present) - PR bodies are being written manually. Branch `chore/refresh-stale-trackers-512-p14` refreshes this Current State section, PROGRESS.md header / Known Issues / sprint summary, and CHANGELOG. |
| 2026-05-08/11 | Claude (Claude Code) | Workflow polish (#125), 5.12 DESIGN.md export (#126), P.14 Claude Design pack (#127), release.yml pack publish step (#128) | Retroactive row added 2026-07-02; these four merges post-date the last session-history update. #125: Visual Regression off PR auto-run to manual + nightly cron, Node 22 LTS across workflows, `pnpm test` as final release.yml pre-publish guard, `.claude/worktrees/**` biome ignore. #126: internal `packages/design-md` converter (`presetToDesignMd`), CLI `arcana-ui export-design-md <preset|all>` with `--validate` running `@google/design.md lint`, new CI job validating all 14 presets per PR (0 errors, 5-8 advisory warnings each). #127: `@arcana-ui/claude-design-pack` package with 14 pre-generated DESIGN.md files + `manifest.claude-design.json`, `regenerate:check` CI drift guard, playground `/claude-design` page with per-preset copy/download and import walkthrough. #128: claude-design-pack publish step added to release.yml. |
| 2026-05-07 | Claude (Claude Code) | Tree-shaking fix (5.9) — per-component entry points and per-subpath exports map | Branch `perf/5.9-per-component-entry-points` off develop after the cleanup PR (#122) merged. Refactored `packages/core/tsup.config.ts` to discover entries programmatically: 9 primitives + 10 composites + 47 patterns + 2 components + 1 layout + 9 hooks + ThemeProvider + cn + version + main barrel index = 82 entries. Each entry's directory in `dist/` (e.g. `dist/primitives/Button/`, `dist/hooks/useTheme.*`) has its own self-contained `.mjs` / `.js` / `.d.mts` / `.d.ts`. Kept `splitting: false` deliberately — splitting would emit shared chunks without the `"use client"` banner that Next.js App Router / Server Components needs. With splitting off and the banner set via `esbuildOptions(options) { options.banner = { js: '"use client"' }; }`, every one of the 82 generated `.mjs` files starts with `"use client"`. Verified by piping `find dist -name "*.mjs" -exec head -1 {} \\; \| sort -u` and confirming a single line of output. `packages/core/package.json` `exports` map gained 81 friendly subpath entries: `./Button`, `./Card`, `./useTheme`, `./ThemeProvider`, `./cn`, etc., each pointing at the corresponding `dist/<category>/<Name>/index.mjs` (or `dist/hooks/<useFoo>.mjs` for hooks). Existing `.`, `./styles`, `./manifest.ai.json`, `./dist/*`, `./package.json` entries preserved unchanged. `sideEffects` widened from `["*.css"]` to `["**/*.css", "**/*.module.css"]` so consumer bundlers preserve every CSS import as a side effect during tree-shaking (otherwise per-component styles would be silently dropped). Bumped `packages/core/package.json` to `0.1.2`. Bear publishes. End-to-end consumer verification at `/tmp/treeshake-test/`: `pnpm pack` from `packages/core` produced `arcana-ui-core-0.1.2.tgz`; npm-installed it into a fresh fixture with two source files: `import { Button } from '@arcana-ui/core'` (full barrel) and `import { Button } from '@arcana-ui/core/Button'` (deep import); bundled both with esbuild minified ESM with React externalized. Result: full barrel = 166.9 kB, deep import = 2.2 kB. **99.2% reduction for the deep-import path.** Original baseline was 278 kB for both. CHANGELOG `[Unreleased]` documents the change with code samples and the before/after numbers. PROGRESS.md marks 5.9 complete and Tree-shaking moved to RESOLVED in Known Issues. Verified: `pnpm lint` passes (auto-formatted package.json after the JSON write didn't match biome's style; no errors), `pnpm test` 982/982 pass, `pnpm build` green across all 12 workspace projects. |
| 2026-05-07 | Claude (Claude Code) | Post-pause audit + cleanup PR (#122 merged) (`chore/cleanup-post-0.1.1`) | Bear returned after a 3-week pause; ran the read-only audit per `CC_AUDIT_KICKOFF.md` and discovered three of the four "open" items in `AUDIT_2026-05-07.md` had already shipped in the 2026-04-15-17 sprint (KV→Supabase #114, three-PR landing polish #115/#116/#117, useTheme tests now passing). Also surfaced: v0.1.1 hotfix for `@arcana-ui/core` shipped 2026-04-17 (PRs #120/#121, CSS module class-name maps were empty in the 0.1.0 published bundle), `develop` was 1 commit behind `main` (the 0.1.1 release merge `0b4ecbe` only on main), `.github/workflows/release.yml` had an uncommitted `continue-on-error: true` change on tokens / core publish steps, biome warnings appeared to double from 77 to 156 but ~78 of those are `.claude/worktrees/` double-counts, and `pnpm audit` reports 20 advisories rooted at transitive `hono@4.12.12` via `@modelcontextprotocol/sdk`. Cleanup PR: synced develop with main (fast-forward to `0b4ecbe`, pushed), branched `chore/cleanup-post-0.1.1` off the synced develop, reverted release.yml on main per Bear's "failed publishes stay loud" call, refreshed `PROGRESS.md` + this CLAUDE.md "Current State" to reflect 0.1.1 + the April sprint + the new P.14 (Claude Design pack) and 5.12 (DESIGN.md export) line items driven by `~/Documents/Claude/Projects/Arcana/COMPETITIVE_INTEL_2026-05-07.md`. KNOWN_ISSUES.md now documents the hono advisories with the failed `pnpm.overrides` attempt (pnpm v10.31.0 wouldn't propagate the override to the lockfile even after a 24-min `--force` install; reverted, deferred until upstream MCP-SDK ships a patched-hono release), the biome worktree double-counting, and marks the useTheme entry as RESOLVED. No code changes to `packages/core` or `packages/tokens` in this PR; tree-shaking fix lives on its own branch (`perf/5.9-per-component-entry-points`) immediately after this cleanup merges. Verified: `pnpm lint` 0 errors / 156 warnings (78 inside the working tree, 78 from worktrees as documented), `pnpm test` 982/982 pass, `pnpm build` green across every workspace project. |

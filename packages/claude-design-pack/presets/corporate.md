...
version: alpha
name: Corporate
description: Conservative navy/slate palette - trustworthy, enterprise-grade, Stripe-inspired
colors:
  primary: "#1d4ed8"
  secondary: "#f5f5f4"
  tertiary: "#1d4ed8"
...
Comfortable density: 16px base, 44px touch targets, professional rhythm.

## Colors

Corporate centers on a single `primary` accent (`#1d4ed8`) on a `background` of `#ffffff`. Every accent / surface / foreground pair below has been measured against WCAG 2.1 contrast ratios; ratios at or above 4.5:1 clear AA at body size.

### WCAG contrast pairs

| Pair | Foreground | Background | Ratio | AA body | AA large |
| --- | --- | --- | --- | --- | --- |
| foreground on background | `#0f172a` | `#ffffff` | 17.85 | pass | pass |
| foreground on surface | `#0f172a` | `#f8fafc` | 17.06 | pass | pass |
| foreground-muted on background | `#94a3b8` | `#ffffff` | 2.56 | fail | fail |
| on-primary on primary | `#ffffff` | `#1d4ed8` | 6.70 | pass | pass |
| border on background | `#e2e8f0` | `#ffffff` | 1.23 | fail | fail |

## Typography

Display set in **Inter** for headline weight and presence; body in **Inter** for sustained reading; **IBM Plex Mono** for code, telemetry, and tabular data. Sizes use `clamp()` for fluid scaling between 320px and 1536px viewports so layouts settle without breakpoint-specific overrides.
...
Reference values: `sm = 0 1px 2px 0 rgba(15, 23, 42, 0.06)`, `md = 0 4px 6px -1px rgba(15, 23, 42, 0.08), 0 2px 4px -2px rgba(15, 23, 42, 0.06)`, `lg = 0 10px 15px -3px rgba(15, 23, 42, 0.08), 0 4px 6px -4px rgba(15, 23, 42, 0.04)`. Cards default to `sm`, hover lifts to `md`, modals sit at `xl`.
